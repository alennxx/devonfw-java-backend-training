...
public class UcFindOrderImpl extends AbstractOrderUc implements UcFindOrder {
  ...
  public List<OrderEto> findOrdersByCreationDateAndStatus(LocalDate creationDate, OrderStatus status) {
    ...
    LOG.debug("Get Orders with creationDate {} and status {} from database.", creationDate, status);
    List<OrderEntity> orders = getOrderRepository().findAllByCreationDateAndStatus(creationDate, status);
