...
public class UcManageOrderImpl extends AbstractOrderUc implements UcManageOrder {
  ...
  public OrderCto saveOrder(OrderCto order) {
    ...
    CustomerEntity ownerEntity = getCustomerRepository()
        .save(getBeanMapper().map(order.getOwner(), CustomerEntity.class));
    List<ItemEntity> positionEntities = getItemRepository()
    ...
    orderEntity.setOwnerId(ownerEntity.getId());
    orderEntity.setOrderPositions(new HashSet<>(positionEntities));
    orderEntity = getOrderRepository().save(orderEntity);

    OrderCto resultOrder = new OrderCto();
    resultOrder.setOrder(getBeanMapper().map(orderEntity, OrderEto.class));
    resultOrder.setOrderPositions(new HashSet<>(getBeanMapper().mapList(positionEntities, ItemEto.class)));
    resultOrder.setOwner(getBeanMapper().map(ownerEntity, CustomerEto.class));
    return resultOrder;
